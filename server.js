...
// ⭐ [修正 1]：加入 materialDetails，讓系統記得每個 ID 的中文名稱與單位
        ...
        if (report.isNoWork) {
            ...
            if (Array.isArray(report.materialItems)) {
                report.materialItems.forEach(item => {
                    ...
                    const key = item.materialId || `${materialName} (${baseUnit})`;
                    stats.materialStats[key] = (stats.materialStats[key] || 0) + baseQuantity;
                    // ⭐ 新增這行：記錄翻譯用的名稱與單位
                    stats.materialDetails[key] = { name: materialName, unit: baseUnit };
                });
            }
        }
    }
...
app.get('/api/projects/:projectId/material-balances', async (req, res) => {
    try {
        ...
        const customConfig = await readProjectMaterials(project.projectName);
        const globalMaterials = await readGlobalMaterials();
        const inventoryMap = {};
        (globalMaterials?.materials || []).forEach(m => inventoryMap[m.materialId] = m);
        (customConfig?.materials || []).forEach(m => inventoryMap[m.materialId] = m);
    ...
    } catch (error) { return res.status(500).json({ success: false, error: '取得餘額失敗' }); }
});
