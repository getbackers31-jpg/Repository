...
async function getGraphClient() {
    ...
    graphClientPromise = (async () => {
        try {
            ...
            tokenExpiresAt = Math.max(Date.now() + 60 * 1000, (response.expiresOn ? response.expiresOn.getTime() : Date.now() + 50 * 60 * 1000) - 5 * 60 * 1000);
...
function validateProjectName(projectName) {
    ...
    if (/[<>:"/\\|?*#%]/.test(normalizedName)) throw new Error('名稱不可含特殊字元');
...
async function readJsonFromOneDrive(filePath, defaultData, throwOnNotFound = false) {
    try {
        ...
        if (!downloadUrl) throw new Error('未回傳下載網址');
...
function verifyLineSignature(rawBody, signature) {
    ...
    // 檢查 Buffer 長度，避免 timingSafeEqual 拋出錯誤
...
app.get('/api/projects', async (req, res) => {
    try {
        const c = await readProjectsFromOneDrive();
        const active = (c.projects || []).filter(p => p.active).map(p => ({ 
            projectId: p.projectId, 
            projectName: p.projectName 
        })).sort((a, b) => a.projectName.localeCompare(b.projectName, 'zh-Hant'));
        res.status(200).json({ success: true, projects: active });
    } catch (e) { 
...
app.get('/api/projects/:projectId', async (req, res) => {
    try {
        const p = await findProjectById(req.params.projectId);
...
app.get('/api/materials', async (req, res) => {
    try {
        const p = await findProjectById(req.query.projectId);
        if (p) {
            const custom = await readProjectMaterials(p.projectName);
            if (custom?.items) return res.status(200).json({ success: true, materials: custom.items });
        }
        const g = await readGlobalInventory();
        res.status(200).json({ success: true, materials: g.items || g.materials || [] });
    } catch (e) { 
        res.status(500).json({ success: false }); 
    }
});
...
app.post('/api/submit-report', async (req, res) => {
    try {
        ...
        if (!project) return res.status(400).json({ success: false, error: '案場不存在' });
        ...
        const invMap = {};
        (globalInv?.items || []).forEach(m => invMap[m.materialId] = m);
        (customInv?.items || []).forEach(m => invMap[m.materialId] = m);

        // ============================
        // 處理材料進場模式
        // ============================
        if (formType === 'material_issue') {
            if (!reportData.materialItems?.length) return res.status(400).json({ success: false, error: '請選擇進場材料' });
...
app.get('/api/projects/:projectId/export-excel', async (req, res) => {
    try {
        ...
        if (!project) return res.status(404).json({ success: false, message: '找不到此專案' });
        ...
        const inventoryMap = {};
        (inventoryData?.items || []).forEach(m => inventoryMap[m.materialId] = m);
        ...
        (transactionData.transactions || []).forEach(tx => {
            ...
            if (!materialSummaryMap[uniqueKey]) {
                materialSummaryMap[uniqueKey] = { 
                    ...
                    issuedQty: 0, baseIssuedQty: 0, consumedBaseQty: 0, baseUnit: tx.baseUnit 
                };
            }
            ...
            (report.materialItems || []).forEach(item => {
                ...
                if (!materialSummaryMap[uniqueKey]) {
                    materialSummaryMap[uniqueKey] = { 
                        ...
                        issuedQty: 0, baseIssuedQty: 0, consumedBaseQty: 0, baseUnit: item.baseUnit 
                    };
                }
